...
public abstract class RotaryDriverBase extends ABaseData {
    protected boolean hasBothBottles() {
        var data = DataManager.get(player);
    ...
    public void summonEffectEntity() {
        var data = DataManager.get(player);
        CompoundTag driverTag = data.getDriverTag();
        ...
        IFullBottle Inorganic = BuildRegistry.FullBottlefindByItemStack(inorganic);
        IFullBottle Organic = BuildRegistry.FullBottlefindByItemStack(organic);
    ...
    private void InorganicEntity(IFullBottle bottle){
        InorganicEntity inorganicEntity = new InorganicEntity(player.level, BuildRegistry.isBestMatch(player), bottle.getBuildArmor().getTexture(),player);
        Build.LOGGER.debug("生成实体{}",inorganicEntity.getCustomName());
    ...
    private void OrganicEntity(IFullBottle bottle){
        OrganicEntity organicEntity = new OrganicEntity(player.level, BuildRegistry.isBestMatch(player), bottle.getBuildArmor().getTexture(),player);
        Build.LOGGER.debug("生成实体{}",organicEntity.getCustomName());
    ...
    public void HenshinBeforeEquieBottleArmor(boolean isHenshinBefore){
        ...
        if (isHenshinBefore) {
            // 根据 注册内容判断
            ArmorUseHandler.saveArmor(player);
    ...
    public boolean isHenshining(IFullBottle iFullBottle , ItemStack itemStack){
        ...
        return iFullBottle.getFullBottle().equals(itemStack.getItem()) && !onFullBottle && data.isHenshin();
    }
    ...
    public void Summon(String Animation_build_up,String Animation_build_up_close,String Animation_build_up_over){
        if (getEffectEntity() instanceof BuildUpEffectEntity) {
            ...
            DelayedTask.chain(player.level)
                    ...
                    .then(1, () -> BuildRegistry.playSound(player, inorganic, organic))
                    .start();
        }
    }
    ...
    public <T extends Entity> List<T> getEntityList(Class<T> EntityClass, double value){
        ...
        // 获取世界实体类型(Class)  目标实体.class    获取范围
        return player.level.getEntitiesOfClass(EntityClass, aabb);
    }
    public void addEntity(IFullBottle bottle){
        final OrganicEntity organicEntity = new OrganicEntity(player.level, BuildRegistry.isBestMatch(player), bottle.getBuildArmor().getTexture(),player);
